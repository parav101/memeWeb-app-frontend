...
const Feed: React.FC = () => {
  ...
  const { loading, memes, error } = useAppSelector(
    (state) => state.memesGet
  );

  useEffect(() => {
    dispatch(getMemes()as any);
  }, [dispatch]);
  ...
  const submitMemeHandler = () => {
    if (
      ...
      dispatch(createMeme(newMemeLink)as any);
      setNewMeme('');
    }
  };
  ...
  const uploadMemeFileHandler = async (e: any) => {
    ...
    try {
      ...
      if (data) {
        setUploading(false);
        dispatch(createMeme(data)as any);
      }
    } catch (err) {
      console.error(err);
    }
  ...
  return (
    <Box w="full" h="auto">
      ...
      {/* {createMe ? <Alert>{error}</Alert> : null} */}
      {loading ? (
        <Loader />
      ) : error ? (
        <Alert status="error">
